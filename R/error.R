...
tsjson_parse_error_cnd <- function(table, text, call = caller_env()) {
  call <- frame_get(call, sys.call)
  cnd <- structure(
    list(
      call = call,
    ...
    class = c("tsjson_parse_error", "error", "condition")
  )
  cnd$message <- paste0(
    format_tsjson_parse_error_(cnd),
    collapse = "\n"
  )
  cnd
}
...
format_tsjson_parse_error_ <- function(x, n = 2, ...) {
...
format_tsjson_parse_error_1 <- function(text, file, row, col, ecol) {
  head <- sprintf("JSON parse error `%s`:%d:%d", file, row, col)
...
format.tsjson_parse_error <- function(x, ...) {
  c("<tsjson_parse_error>", format_tsjson_parse_error_(x, ...))
}

#' @export

print.tsjson_parse_error <- function(x, ...) {
  writeLines(format(x, ...))
  invisible(x)
}
